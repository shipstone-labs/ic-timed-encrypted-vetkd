...
shared ({ caller = initializer }) actor class () {
    ...
    private func is_authorized(user : PrincipalName, note : EncryptedNote) : Bool {
        user == note.owner or Option.isSome(Array.find(note.users, func(x : PrincipalEntry) : Bool { 
          (Option.isNull(x.name) or (Option.get(x.name, "") == user))
            and (Option.isNull(x.when) or (Option.get(x.when, 0) <= Time.now()))
      }));
    };
    ...
    public shared ({ caller }) func create_note(data : Text) : async NoteId {
    ...
    public shared ({ caller }) func get_notes() : async [EncryptedNote] {
        ...
        let shared_notes = List.filter(List.map(
            Option.get(noteIdsByUser.get(user), List.nil()),
            func(nid : NoteId) : EncryptedNote {
                expect(notesById.get(nid), "missing note with ID " # Nat.toText(nid));
            },
        ), func(note : EncryptedNote) : Bool {
          let fn = func(u : PrincipalEntry) : Bool {
            (Option.get(u.name, "") == user) and (Option.isNull(u.when) or (Option.get(u.when, 0) <= Time.now()))
          };
          not Option.isNull(Array.find(note.users, fn))
        });
        let public_notes = List.filter(List.map(
            Option.get(noteIdsByUser.get("everyone"), List.nil()),
            func(nid : NoteId) : EncryptedNote {
                expect(notesById.get(nid), "missing note with ID " # Nat.toText(nid));
            },
        ), func(note : EncryptedNote) : Bool {
          let fn = func(u : PrincipalEntry) : Bool {
            Option.isNull(u.name) and (Option.isNull(u.when) or (Option.get(u.when, 0) <= Time.now()))
          };
          not Option.isNull(Array.find(note.users, fn))
        });

        let buf = Buffer.Buffer<EncryptedNote>(List.size(owned_notes) + List.size(shared_notes) + List.size(public_notes));
        buf.append(Buffer.fromArray(List.toArray(owned_notes)));
        buf.append(Buffer.fromArray(List.toArray(shared_notes)));
        buf.append(Buffer.fromArray(List.toArray(public_notes)));
        Buffer.toArray(buf);
    };
    ...
    public shared ({ caller }) func update_note(id : NoteId, data: Text, encrypted_text : Text) : async () {
        assert not Principal.isAnonymous(caller);
        ...
        let updated_note = { note_to_update with encrypted_text };
        notesById.put(id, { updated_note with data});
    };
    ...
    public shared ({ caller }) func add_user(note_id : NoteId, user : PrincipalEntry) : async () {
        assert not Principal.isAnonymous(caller);
        let caller_text = Principal.toText(caller);
        ...
        let users_buf = Buffer.fromArray<PrincipalEntry>(note.users);
        users_buf.filterEntries(func(i : Nat, u : PrincipalEntry) : Bool { u.name != user.name });
        users_buf.add(user);
        let updated_note = { note with users = Buffer.toArray(users_buf) };
        notesById.put(note_id, updated_note);
        let user_name = Option.get(user.name, "everyone");
        switch (noteIdsByUser.get(user_name)) {
            case (?user_nids) {
                if (not List.some(user_nids, func(nid : NoteId) : Bool { nid == note_id })) {
                    noteIdsByUser.put(user_name, List.push(note_id, user_nids));
                };
            };
            case null {
                noteIdsByUser.put(user_name, List.make(note_id));
            };
        };
    };
    ...
    public shared ({ caller }) func remove_user(note_id : NoteId, user : ?PrincipalName) : async () {
        ...
        let users_buf = Buffer.fromArray<PrincipalEntry>(note.users);
        users_buf.filterEntries(func(i : Nat, u : PrincipalEntry) : Bool { u.name != user });
        let updated_note = { note with users = Buffer.toArray(users_buf) };
        notesById.put(note_id, updated_note);

        let user_name = Option.get(user, "everyone");
        switch (noteIdsByUser.get(user_name)) {
            case (?user_nids) {
                let updated_nids = List.filter(user_nids, func(nid : NoteId) : Bool { nid != note_id });
                if (not List.isNil(updated_nids)) {
                    noteIdsByUser.put(user_name, updated_nids);
                } else {
                    let _ = noteIdsByUser.remove(user_name);
                };
            };
            case null {};
        };
